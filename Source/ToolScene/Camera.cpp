...
namespace
{
	...
	const VECTOR3 ADD_HEIGHT = LOOK_HEIGHT + VECTOR3(0.0f, 20.0f, 0.0f); // userの頭より少し上を見る
...
void Camera::SetUserPosition(const Transform& transform)
{
	user_ = transform;
}
...
{
	cameraPosition_ = user_.position_ + LOOK_HEIGHT; // 目線の高さに合わせている
	targetPosition_ = user_.position_ + LOOK_HEIGHT + VECTOR3(0, 0, 1) * FIRST_DISTANCE * MGetRotY(user_.rotation_.y);
}
...
void Camera::ThirdUpdate()
{
	...
	cameraPosition_ = user_.position_ + camPos + LOOK_HEIGHT;
	targetPosition_ = user_.position_ + ADD_HEIGHT;
